...
spiders %>% 
  ...
  summary() #boundary of the map(min, max)
```
...
```{r}
map1 <- get_stadiamap(bbox, maptype="stamen_terrain", zoom = 7) #zoom: quality of the map(higher the zoom number, more detail)
```
...
```{r}
ggmap(map1)+
  geom_point(data=spiders, aes(longitude, latitude), size=0.75, color="black", alpha=0.8)+. #size: size of the point
...
```{r}
ggmap(map2)+
  geom_point(data=grizzly, aes(longitude, latitude), size=0.75, color="black", alpha=0.7)
```

...
A numeric vector.
```{r}
my_vector <- c(10, 20, 30) #this makes a new vector. # each vector is a its own column in the data frame. columns=variables
```
...
The first row.  
```{r}
hbirds <- tibble(sex=Sex, length=Length, weight=Weight) #change the name of variables
hbirds
```

The third column.  
```{r}
hbirds[1,] #get the first row only
```
```{r}
hbirds[1,3] #[row, column]
```
```{r}
hbirds[,3] #only third column
```
...
#row.names=FALSE: prevents R from saving row numbers as a fake column in your CSV file
```
```

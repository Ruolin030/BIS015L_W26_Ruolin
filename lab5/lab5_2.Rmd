...
```{r}
filter(penguins, species=="Adelie") #(data, column=="row") # 2==
```
...
```{r}
filter(penguins, species!="Chinstrap") #remove "Chinstrap"
```
...
```{r}
filter(penguins, body_mass_g %in% c(5250, 5500)) #look for specific values, 5200 and 5500
```

Alternatively, you can use `between` if you are looking for a range of specific values.  
```{r}
filter(penguins, between(body_mass_g, 5250, 5500)) #between: a range from 5250 to 5500
```

How would you use `%in%` to get the same result?
```{r}
filter(penguins, body_mass_g %in% c(5250, 5300, 5350, 5400, 5450, 5500)) #need to type each specific number we need
```

You can also extract observations "near" a certain value but you need to specify a tolerance.  
```{r}
filter(penguins, near(body_mass_g, 5400, tol=100)) #tol=tolerate #tol: near(5350-5450) splits to 50
```
...
```{r}
filter(penguins, species=="Chinstrap" & body_mass_g> 4500)  #&: and
```

Notice that the `|` operator generates a different result. Why?
```{r}
filter(penguins, species=="Chinstrap" | body_mass_g> 4500) #|: or
```
...
```{r}
filter(penguins, body_mass_g>5000, (bill_length_mm>50|bill_depth_mm>18)) #,stands for &
```
...
```{r}
penguins %>%      #shift+command+m: %>%   #select goes first when using %>%  
  select(species, body_mass_g) %>% #first select columns
  filter(species=="Gentoo") #then select the specific row
```
...
penguins %>%     
  ...
  arrange(body_mass_g) #ascending order
```
...
penguins %>%     
  select(species, body_mass_g) %>% 
  filter(species=="Gentoo") %>% 
  arrange(desc(body_mass_g)) #descending order: desc
```

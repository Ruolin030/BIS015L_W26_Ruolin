...
```{r}
glimpse(penguins) #dbl stands for double precision floating point number(it's a type of numeric variable). int: integer. chr: character
penguins
```
...
select(): pull out columns of interest from a dataframe
```{r}
names(penguins)
```
...
```{r}
select(penguins, species, body_mass_g) #show x and y
```
...
```{r}
select(penguins, species:flipper_length_mm) # : stands for between(selects a continuous range of columns, e.x. select data from species to flipper_length_mm)
```

The ! operator is useful in select. It allows us to select everything except the specified variables.
```{r}
select(penguins, !body_mass_g) #! stands for remove
```
...
```{r}
select(penguins, where(is.numeric)) #pull out all the numerical date #numeric is a class of data
```
...
```{r}
rename(penguins, body_mass=body_mass_g) #how to restore it in dataframe?
```
...
```{r}
relocate(penguins, year) #remove the column to the first column
```
...
```{r}
msleep_focused <- select(msleep, order, genus, bodywt) #msleep_focused: output data
```

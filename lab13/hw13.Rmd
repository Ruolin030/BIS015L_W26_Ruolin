...
```{r}
amniota_no_na <- read_csv("data/amniota.csv", na="-999") 
```
...
amniota_no_na <- amniota_no_na %>% 
  replace_with_na(replace = list(female_maturity_d=-30258.71100))
```
...
amniota_no_na %>% 
  ...
  miss_var_summary()
```
Yes, this result for mammalia makes sense biologically; because mammals do not lay eggs, they don't have values for the egg mass. For birds and reptiles, NA's in egg mass may represent missing data.

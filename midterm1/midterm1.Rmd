...
Link to repository:
https://github.com/Ruolin030/BIS015L_W26_Ruolin
...
anolis %>% 
  ...
  labs(title="the number of anole species from each island", x="island", y="the number of anole species")
```
...
anolis %>% 
  ...
  labs(title="the number of anole species sampled from each habitat type", x="habitat", y="the number of anole species")
```
...
```{r}
anolis %>% 
  ggplot(mapping=aes(x=hindlimb_length_mm, y=habitat))+
  geom_boxplot(mapping=aes(fill=habitat))+
  labs(title="hindlimb length by habitats", x="hindlimb length (mm)")
```
...
```{r}
anolis %>% 
  ggplot(mapping=aes(x=ratio_of_hindlimb_to_body, y=habitat))+
  geom_boxplot(mapping=aes(fill=habitat))+
  labs(title="ratio of hindlimb to body by habitat types", x="ratio of hindlimb to body", y="habitat")
```
The ratio of hindlimb to body on Trunk-ground is greater and the ratio of hindlimb to body on Trunk-crown is less.
...
anolis %>% 
  ...
  labs(title="body length by tail length", x="body length (mm)", y="tail length (mm)")
```
As the body length increases, the tail length increases too.
Lizards in the Grass-bush have longer tail and relatively shorter body length.
...
anolis %>% 
  ...
  summarize(toepad_lamellae_t_ground=mean(toepad_lamellae_count))
```

```{r}
anolis %>% 
  filter(habitat=="Twig") %>% 
  summarize(toepad_lamellae_twig=mean(toepad_lamellae_count))
```

```{r}
anolis %>% 
  filter(habitat=="Grass-bush") %>% 
  summarize(toepad_lamellae_g_bush=mean(toepad_lamellae_count))
```

```{r}
anolis %>% 
  filter(habitat=="Trunk-crown") %>% 
  summarize(toepad_lamellae_t_crown=mean(toepad_lamellae_count))
```
...
anolis %>% 
  filter(habitat=="Trunk-crown") %>% 
  ggplot(mapping=aes(x=toepad_lamellae_count, y=island))+
  geom_boxplot(mapping=aes(fill=habitat))+
  labs(title="toepad lamellae by island for trunk-crown species", x="the number of toepad lamellae", y="islands")
```
This is not consistent across all islands. 

...
ggplot(data=penguins,
  ...
  geom_point()+
  geom_smooth(method=lm, se=F)+
  labs(title="body mass by flipper length", x="body mass(g)", y="flipper length(mm)")
```
...
```{r}
penguins %>% 
  mutate(body_mass_kg=body_mass_g/1000) %>%  #new_column=old_column # mutate: add the created new column to the end of the dataframe
  select(species, body_mass_g, body_mass_kg) %>%  #mutate: keeps original + adds new columns
...
```{r}
mammals %>% 
  mutate(across(c("order", "family"), tolower)) #work on specific column
```
...
mammals %>% 
  select(genus, species, newborn) %>% 
  mutate(newborn_new=ifelse(newborn == -999.00, NA, newborn)) #if newborn=-999 replaces to NA, else, keep "newborn"
```

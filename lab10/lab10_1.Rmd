...
`geom_bar` #used for count
```{r}
life_history %>% 
  ggplot(mapping=aes(order))+
  geom_bar()+
  coord_flip()
```
...
life_history %>% 
  ...
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  arrange(desc(mean_mass)) %>% 
  ggplot(mapping=aes(x=reorder(order, mean_mass),  y=mean_mass))+ #Sort the orders by mean_mass
  geom_col()+
  coord_flip()+
  scale_y_log10()
```

## Scatterplots #compare two continuous variables 
...
```{r}
life_history %>% 
  ggplot(mapping=aes(x=log10(mass), y=order))+
  geom_boxplot(na.rm = T, mapping=aes(fill=order))
```
...
```{r}
life_history %>% 
  ggplot(mapping=aes(x=log(mass), y=litter_size))+
  geom_point()+
  geom_smooth(method=lm, se=F, na.rm=T)
```
...
life_history %>% 
  ...
  scale_x_log10()+
  labs(title="Mass vs. Litter size", x="Mass(g)", y="Litter Size")+
  theme(plot.title=element_text(size=rel(2), hjust=.5)) #size: front size; rel: relative size; hjust: horizontal justification(left/center/right)
```

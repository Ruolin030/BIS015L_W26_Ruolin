...
```{r}
heartrate_long <- heartrate %>% #DON"T FORGET RESTORE IT
  pivot_longer(-patient,  #-: don't remove patient
               names_to="drug", #columns' name
               values_to="heartrate") 
heartrate_long
```
...
relig_income_long <- relig_income %>% 
  pivot_longer(-religion,
               names_to = "income", #give a new column name
               values_to = "total") #move the count into a new column
relig_income_long
```
  #Don'know/refused should be NA.???
...
```{r}
billboard %>% 
  pivot_longer(wk1:wk76, #wk1 through w76
               names_to="week",
               values_to="rank",
               values_drop_na = T)
```
...
billboard %>% 
  pivot_longer(
    cols=starts_with("wk"), #Select all columns whose names start with wk, and pivot those.
    names_to = "week",
...
```{r}
qpcr_tidy %>% 
  separate(experiment, into=c("experiment", "e_number"), sep=3) %>% # sep=3 separate at the third position
...
qpcr_tidy %>% 
  ...
  separate(replicate, into=c("replicate", "r_number"), sep=3) %>% 
  select(gene, experiment=e_number, replicate=r_number, `mRNA expression`) # using ``: there is space bewtween mRNA and expression
```

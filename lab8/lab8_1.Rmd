...
```{r}
elephants %>% 
summarise(across(where(is.numeric),   #~mean: using function`mean`; .x: on every cell x
                 ~mean(.x, na.rm=T))) #it won't change names of variables. names are actually the mean of age, height, length and circumference.
```
...
```{r}
elephants %>% 
  group_by(sex) #group_by: only group by categorical variables; NOT group by a continuous data
```
...
elephants %>% 
  ...
  summarise(mean_estimated_age_years=mean(estimated_age_years, na.rm=T),
                                          n=n(),
            .groups="keep") %>%  #.group="keep": keep the group
  arrange(sex)
```
